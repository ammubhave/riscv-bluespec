...
class ProcIndication : public ProcIndicationWrapper {
 public:
    ...
    virtual void imem_req(uint8_t op, uint64_t addr, uint64_t data) {
        if (op == false) {  // Ld
            printf("imem_req ld: %p %p\n", (void*)addr, (void*)mem[addr / 8]);
    ...
    virtual void dmem_req(uint8_t op, uint64_t addr, uint64_t data) {
        if (op == false) {  // Ld
            ...
            mem[addr / 8] = data;
            /*addr /= 8;
            uint8_t* bytesIn = reinterpret_cast<uint8_t*>(&data);
            uint8_t* bytes = reinterpret_cast<uint8_t*>(&mem[addr]);
            for(int i = 0; i < 8; i++)
              if(byteEn[i])
                bytes[i] = bytesIn[i];
            mem[addr] = *(reinterpret_cast<uint64_t*>(bytes));*/
        }
    }
#else
