...
{
  size_t npage = ((length-1)>>RISCV_PGSHIFT) + 1 /*/RISCV_PGSIZE+1*/;
  if (flags & MAP_FIXED)
  {
...
{
  uintptr_t n = ROUNDUP(len, RISCV_PGSIZE) >> RISCV_PGSHIFT;// / RISCV_PGSIZE;
...
#define first_free_paddr() (sbi_top_paddr() + num_harts * RISCV_PGSIZE + 0x400000) // AMOLOS

void vm_init()
{
  current.first_free_paddr = first_free_paddr();
  //printk("current.first_free_paddr:       %p\n", current.first_free_paddr);
...
void supervisor_vm_init()
{
  ...
  for (size_t i = 0; i < num_middle_pts; i++) {
    ...
    //printk("root_pt[] %p = %p\n", &root_pt[(1<<RISCV_PGLEVEL_BITS)-num_middle_pts+i], root_pt[(1<<RISCV_PGLEVEL_BITS)-num_middle_pts+i]);
  }
#endif
  memset(middle_pt, 0, root_pt - middle_pt + RISCV_PGSIZE);

  //printk("middle_pt = %p, root_pt = %p\n", middle_pt, root_pt);
  for (uintptr_t vaddr = current.first_user_vaddr, paddr = vaddr + current.bias, end = current.first_vaddr_after_user;
    ...
    //printk("middle_pt[l2_idx] %p = %p\n", &middle_pt[l2_idx], middle_pt[l2_idx]);
  }
  current.first_vaddr_after_user += (void*)root_pt + RISCV_PGSIZE - (void*)sbi_pt;

  // map SBI at top of vaddr space
  uintptr_t num_sbi_pages = sbi_top_paddr() >> RISCV_PGSHIFT;// / RISCV_PGSIZE;
