...
long handle_interrupt(long cause, long epc, long long regs[32])
{
  if (cause == IRQ_TIMER) {
    handle_frontend_syscall(SYS_write, '.', 0, 0);
    int tim = read_csr(stime);
    write_csr(stimecmp, tim + 20);
  }
}
...
long handle_trap(long cause, long epc, long long regs[32])
{
  ...
  if (cause < 0)
  {
    handle_interrupt((cause << 1) >> 1, epc, regs);
    return epc;
  }
  else
  {
    ...
    else if (regs[17] == SYS_sti) {
      int tim = read_csr(stime);
      write_csr(stimecmp, tim + 10);
